/* _____________ LV1 크기가 작은 부분 문자열 _____________ */
...
/*
  ...
  ** 입출력 예 **
  ...
  - 입출력 예 #2
    p의 길이가 1이므로 t의 부분문자열은 "5", "0", 0", "2", "2", "0", "8", "3", "9", "8", "7", "8"
    이며 이중 7보다 작거나 같은 숫자는 "5", "0", "0", "2", "2", "0", "3", "7" 이렇게 8개가 있습니다.
...
function solution(t, p) {
  ...
  for(let i = 0; i < t.length; i++) {
      if (t.substr(i, p.length).length == p.length) {
          let formattedStr = t.substr(i, p.length)
          if(parseInt(formattedStr) <= parseInt(p)) {
