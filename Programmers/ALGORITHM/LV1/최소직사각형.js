/* _____________ LV1 최소직사각형 _____________ */
...
/*
  ...
  ** 제한 사항 **
  ...
  - sizes의 길이는 1 이상 10,000 이하입니다.
    ㄴ sizes의 원소는 [w, h] 형식입니다.
    ㄴ w는 명함의 가로 길이를 나타냅니다.
    ㄴ h는 명함의 세로 길이를 나타냅니다.
    ㄴ w와 h는 1 이상 1,000 이하인 자연수입니다.
...
function solution(sizes) {
  sizes.forEach((v, i) => {
      sizes[i] = v.sort((a, b) => a - b);
  })
  
  const horizontal = sizes.map((v) => v[0]);
  const vertical = sizes.map((v) => v[1]);
