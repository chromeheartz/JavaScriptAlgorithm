...
/*
  * 브라우저에 URL을 입력하면 무슨 일이 발생할까? *
  ...
  브라우저는 DNS로 요청을 보내기전에 몇가지 체크를 한다.
  해당 도메인을 이미 알고있는지 확인 없다면 로컬 컴퓨터의 hosts 파일을 참조
  만약 정의가 되어있다면 내부적으로 IP 반환하고
  ...
  4.2 만약 예시로 든 주소에 택배를 보내려면 어떻게 될까?

  - 먼저 논리적 주소를 보고 접근할것이다. 서울 > 종로구 > 세종로 를 탐색
    탐색 순서가 서울의 진짜 위치, 종로구의 진짜위치, 세종로의 진짜위치를 찾게 된다.
    ARP를 이용했던것과 비슷하다
  - IP는 대역을 통해 범위를 좁혀나간다.
