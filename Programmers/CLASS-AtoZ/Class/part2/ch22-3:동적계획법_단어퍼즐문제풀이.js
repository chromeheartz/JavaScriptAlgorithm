...
/*
  ...
  ** 문제 풀이 **
  ...
  단어 퍼즐은 프로그래머스 레벨 4 문제로 쉽지 않은 문제다.
  문제 설명만 읽었을 때는 순열 / 백트래킹을 통해 풀 수 있을 것처럼 보인다.
  그렇지만 순열은 시간복잡도가 매우 크기 때문에 사용하기 힘들고 백트래킹을 통해 가지치기를 하더라도
  제한 시간을 통과하기는 힘들어보인다.
  따라서 이런 경우엔 동적 계획법으로 해결할 수 있을지 확인해봐야 한다.

  *** 동적 계획법인지 확인하기 ***

  우선 문제를 어떻게 정의해야 할지 정해야 한다.
  이 문제의 경우 필요한 단어 개수의 최소값을 구해야 하기 때문에 가장 작은 문제도 단어 개수로 정의하는 것이 좋다.
  예를 들어, 입출력 예로 들어온 ["ba", "na", "n", "a"], "banana"의 경우 가장 작은 문제를 다음과 같이 정의할 수 있다.
  ...
  먼저 단어 "b"부터 확인. 단어 "b"의 경우 ["ba", "na", "n", "a"]에서 
  가능한 단어조각이 없기 때문에 만드는 것이 불가능하다. 따라서 -1이다.

  단어 "ba"는 ["b", "a"]과 "ba"로 확인할 수 있다. 
  하지만 "b"는 단어 조각으로 만들 수 없기 때문에 "ba"로 확인한다. 
  확인해보면 ["ba", "na", "n", "a"]에서 1개 단어 조각으로 구성이 가능. 따라서 값이 1이다.
  
  단어 "ban"은 ["b", "an"], ["ba", "n"], [ban"으로 나눠서 확인할 수 있다. 
  첫 번째의 경우 "b"가 -1이기 때문에 생략. 두 번째 ["ba", "n"]에서 "ba"는 이미 1인 것을 
  구했기 때문에(메모이제이션) "n"만 구하면 됩니다. "n"은 1개 단어 조각으로 구성이 가능하다. 
  따라서 합치면 2개 단어 조각으로 "ban"을 구성할 수 있다. 
  따라서 현재 "ban"의 단어 개수 최솟값은 2다. "ban"은 가능한 단어 조각이 없기에 생략.

  단어 "bana"는 ["b", "ana"], ["ba", "na"], ["ban", "a"], "bana"로 나눠서 확인할 수 있다. 
  되는 것은 ["ba", "na"]만 가능하다. 
  "ba"는 이미 메모이제이션을 통해 1임을 알기에 "na"만 확인하면 된다. 
  "na"는 1개 단어 조각으로 구성이 가능하기에 "bana"는 단어 개수 최솟값이 2가 된다.

  단어 "banan"은 되는 것만 보면 ["bana", "n"]다. "bana"는 이미 2인 것을 알기에 n만 더해서 3이 된다.
  마지막으로 단어 "banana"는 되는 것만 확인하면 ["bana", "na"]와 ["banan", "n"]이다. 
  ["bana", "na"]은 각각 2와 1이 더해져 3이지만 ["banan", "n"]은 3과 1이 더해져 4기 때문에 
  저 작은 값인 ["bana", "na"]을 선택한다.
