...
/*
  ...
  ** 문제 유형 파악하기 **
  ...
  문제 설명에서 핵심적인 부분을 살펴보면 1번 마을에 있는 음식점에서 각 마을로 음식 배달을 하려고 한다.
  N개의 마을 중에서 K 시간 이하로 배달이 가능한 마을 두 가지라 할 수 있다.
  그리고 이미지를 살펴보면 그래프의 간선에 가중치가 있는 것을 확인할 수 있다.

  그렇다면 문제 유형이 어떻게 될까? 
  먼저 K 시간 이하인 마을을 알아내기 위해 1번 마을에서 특정 마을까지의 최단 거리를 알아야 한다.
  그러기 위해서 최단 경로 알고리즘 중 하나를 선택할 수 있다.
  이때 BFS나 DFS를 이용할 수도 있지만 간선에 가중치가 있기 때문에 다익스트라 알고리즘을 이용하는 것이 더 효율적이다.

  **** 문제 풀이 ****

  ** 최소 힙 구현 **

  먼저 다익스트라 알고리즘을 구현하기 위해 힙을 구현해야 한다.
  정점을 낮은 순서로 찾아야 하기 때문에 최소 힙을 구현해야 한다.
  최소 힙은 이전 힙 영상에서 구현한 최대 힙의 조건들을 반대로 구성하면 된다.
  ...
  //- 기본적인 최소 힙 코드이다.
      여기서 최소 힙이 정점과 간선의 값을 담을 수 있도록 받는 자료형을 객체로 변경해본다.
      객체는 필드를 담을 수 있다.
  ...
  //- 이어서 편의를 위해 isEmpty 함수를 작성

  isEmpty() {
    return this.heap.length === 1;
  }

  //- 그리고 pop함수를 다음과 같이 변경
  ...
  ***** 다익스트라 알고리즘 구현 ****
  ...
  힙까지 구현했다면 다익스트라 알고리즘 자체는 크게 어렵지 않다.
  ...
  * 답찾기
  ...
  최종적으로 dist 배열에서 K 이하인 값만 찾고 그 길을 반환하면 된다.
