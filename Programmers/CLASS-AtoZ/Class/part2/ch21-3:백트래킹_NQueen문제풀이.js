...
/*
  ...
  * 전략 *
  ...
  우선 모든 경우의 수를 찾는것은 괜찮다. 그런데 어떻게 가지치기를 해야 할까? 
  가지치기를 할 수 있는 경우는 다음과 같다.

  1. 퀸을 둔 행은 가지치기 한다.
  2. 퀸을 둔 열은 가지치기 한다.
  3. 퀸을 둔 대각선 왼쪽, 오른쪽을 가지치기 한다.

  가지치기 조건을 찾은 것은 좋지만 가지치기를 위해 행과 열, 대각선을 루프를 통해 검사하게 되면
  성능을 크게 낭비하게 된다. 그래서 최대한 적은 비용으로 가지치기를 하기 위해 1차원 배열을 사용.
  1차원 배열은 다음과 같이 데이터가 들어간다.

  - 배열의 index는 행의 위치, 해당 index의 value는 열의 위치
    ㄴ 예를 들어, queen[2] = 1 은 체스판 위에서 두 번째 줄, 첫 번째 칸에 해당한다.(1부터 시작한다고 가정할 때)
  
  위와 같이 데이터 형태를 잡는다면 조건을 다음과 같이 잡을 수 있다.

  - 한 index에 여러 value를 둘 수 없기에 행은 자연스럽게 가지치기 된다.
  - index가 같다면 둘 수 없다. 같다면 가지치기 한다.
  - 행, 열에 대한 차가 같다면 대각선에 있다는 뜻이므로 가지치기 한다.
    ㄴ 예를 들어, 1부터 시작한다고 가정할 때 queen[3] = 2, queen[1] = 4, 일 때
      행에 대한 차 3 - 1과 열에 대한 차 4 - 2는 같기 때문에 대각선에 있다는 뜻이다.
      (절대값으로 계산하면 왼쪽, 오른쪽 둘다 체크 가능하다)
